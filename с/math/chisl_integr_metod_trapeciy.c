#include <stdio.h>  // подключаем библиотеку для ввода-вывода данных в консоль
#include <math.h>   // подключаем библиотеку для работы с математикой
...
int main() { // объявим главную функцию которая оркестрирует выполнение программы, вернем int с кодом выполнения
    ...
    int n_list[] = {15, 20, 150, 650}; // объявим  целочисленный массив для хранения количества разбиений. размер массива вычислится автоматически. заполним его согласно заданию.
    int n_list_size = sizeof(n_list) / sizeof(n_list[0]);  // посчитаем количество элементов в массиве путем деления размера массива в байтах на размер одного элемента массива в байтах, запишем количество элементов в массиве в n_list_size

    double the_most_precise_result = 0.0; // Объявим переменную для хранения самого точного результата вычисления
    int the_biggest_n = 0; // объявим переменную для хранения n при котором получается наиболее точный результат

    for (int i = 0; i < n_list_size; i++) { // создадим цикл для прохода по массиву n_list, цикл выполняется пока счетчик i, совпадающий с индексацией массива не превысит размер массива
        int n = n_list[i]; // возьмем элемент массива согласно счетчику и запишем его в переменную n
        ...
        printf("Определенный интеграл от функции f(x)=√x² * cos_x методом трапеций при a = %.2f, b = %.2f, n = %d равняется: %.6f\n", a, b, n, result); // выведем в консоль результат текущей итерации для текущего n. поставим перенос строки. Спецификаторы-плейсхолдеры в зависимости от типа числа и необходимой точности.
    ...
    printf("\nСамое точное приближенное значение интеграла будет при n = %d и равняется: %.6f\n", the_biggest_n, the_most_precise_result); // Перемемем строку, выведем лучший результат при самом большом n и перенесем строку. Спецификаторы-плейсхолдеры в зависимости от типа числа и необходимой точности.
