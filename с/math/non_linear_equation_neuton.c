#include <stdio.h> // подключим стандартную библиотеку ввода-вывода в С
#include <math.h> // подключим стандартную библиотеку для работы с математикой в С
...
double df(double x) { // объявим функцию df которая будет вычислять производную функции f(x)
    return 1 + (1 / (2 * sqrt(x))) - 2 * sin(2 * x); // вернем результат вычислений математического представления производной функции f(x)
...
double non_linear_eq_by_newton(double x_start, double tol, int max_iter) { // объявим функцию для проведения вычислений по методу Ньютона. Вернем найденный корень уравнения типа double
    ...
    for (int i = 0; i < max_iter; i++) { // задекларируем цикл, который выполнит расчеты по методу Ньютона max_iter итераций
        ...
        if (fabs(dfx) < 1e-10) { // условие, которое сравнивает получившуюся произволную(eё модуль) с очень маленьким числом. метод Ньютона даст ошибку в случае если производная будет близка к нулю 
            printf("Исключительны случай - слишком близкое к 0 значение производной, вычисление по методу Ньютона не даст осмысленного результата.\n"); // сообщим, что у нас исключительный случай
    ...
    printf("За %d итераций не удалось найти корня по методу Ньютона, удовлетворяющему условию порога сходимости.\n", max_iter); // выводим сообщение, что за заданное число итераций не удалось найти корень, удовлетворяющий условиям порога сходимости
...
int main() { // объявляем главную функцию-точку входа в программу
    ...
    if (!isnan(root)) { // условие-проверка на наличие ошибок, в функции non_linear_eq_by_newton мы заложили возврат значения NAN в том случае если чтото идет не так
        printf("Найденный корень: %.6f\n", root);  // выводим строку с найденным корнем
        printf("Результат проверки с найденным корнем = %.6f\n", f(root)); // выводим строку с проверкой - подставим найденный корень
    } else { // ветвление условия
        printf("Корень не найден.\n"); // если у нас NAN то пишем что корень не найден
